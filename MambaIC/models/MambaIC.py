# 파일 이름: MambaIC.py
# (SOTA 원본 - 백본에 활성화 함수 없음, [0, 1] 데이터용)
...
# ❗️ .VSS_module (상대 경로) -> VSS_module (절대 경로)
# (train.py가 프로젝트 루트에 있다는 가정 하에)
from models.VSS_module import conv, deconv, SS2D
...
class SwinBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if (x.size(-1) <= self.window_size) or (x.size(-2) <= self.window_size):
            resize = True # ❗️❗️ 1. 수정: resize=True 추가
            padding_row = (self.window_size - x.size(-2)) // 2
            padding_col = (self.window_size - x.size(-1)) // 2
            x = F.pad(x, (padding_col, padding_col, padding_row, padding_row))
        trans_x = Rearrange('b c h w -> b h w c')(x)
        trans_x = self.block_1(trans_x)
        trans_x =  self.block_2(trans_x)
        trans_x = Rearrange('b h w c -> b c h w')(trans_x)
        if resize: 
            # ❗️❗️ 2. 수정: trans_x를 올바르게 크롭(un-pad)
            trans_x = F.pad(trans_x, (-padding_col, -padding_col, -padding_row, -padding_row))
        return trans_x
...
class MambaIC(CompressionModel):
    ...
    def compress(self, x):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            y_non_anchor_quantilized = torch.zeros_like(means_anchor)
            y_non_anchor_quantilized[:, :, 0::2, 1::2] = non_anchor_quantized[:, :, 0::2, :]
            y_non_anchor_quantilized[:, :, 1::2, 0::2] = non_anchor_quantized[:, :, 1::2, :]

            y_hat_slice = y_anchor_decode + y_non_anchor_quantilized
