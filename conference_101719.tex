...
Section~\ref{sec:system_model} describes the system model and problem formulation. Section~\ref{sec:asym_arch} presents the proposed asymmetric architecture and its structural motivation. Sections~\ref{sec:offline} and~\ref{sec:online} detail the offline policy construction and online reliability-aware selection in RP-MPQ, respectively. Section~\ref{sec:experiments} reports experimental results, followed by concluding remarks in Section~\ref{sec:conclusion}.
...
% =========================================================
\section{Asymmetric CSI Feedback Architecture under UE Constraints}
\label{sec:asym_arch}
% =========================================================
...
\begin{remark}[Polynomial envelopes as Laplace mixtures of exponential modes]
\label{rem:poly_mixture}
For an integer distance index $d\ge 0$, the prototypical long-tail envelope admits an exact Laplace-mixture representation:
\begin{equation}
\frac{1}{d+1}
=
\int_{0}^{\infty} e^{-(d+1)\alpha}\,d\alpha
=
\int_{0}^{\infty} e^{-\alpha}\, e^{-\alpha d}\,d\alpha.
\label{eq:inv_mixture}
\end{equation}
Thus, a slowly decaying envelope can be interpreted as a continuum mixture of exponentially decaying modes in $d$.
A finite-dimensional SSM realizes a finite collection of exponential modes (via the eigenstructure of $\mathbf{A}$), providing an architectural prior compatible with long-tailed locality under bounded UE complexity.
\end{remark}

\subsection{Quantization Robustness of Contractive State-Space Encoders}
\label{subsec:ssm_quant_robust}
UE-side inference often requires low-precision execution. Post-training weight quantization can be modeled as a parameter perturbation $\hat{\boldsymbol{\theta}}=\boldsymbol{\theta}+\Delta\boldsymbol{\theta}$ applied to the UE encoder blocks. The following proposition formalizes a stability property: when the state recursion is uniformly contractive, quantization-induced perturbations do not amplify with token length.

\begin{proposition}[Perturbation robustness of contractive state-space updates]
\label{prop:quant_robust}
Consider a (selective) state-space encoder block written as
\begin{equation}
...
\end{equation}
and let $\hat{\boldsymbol{\theta}}=\boldsymbol{\theta}+\Delta\boldsymbol{\theta}$ denote quantized parameters. Let $(\hat{\mathbf{s}}_t,\hat{\mathbf{y}}_t)$ be the resulting trajectories under the same inputs and identical initialization $\mathbf{s}_0=\hat{\mathbf{s}}_0$. Assume there exist constants $\rho\in[0,1)$ and $L_{\Phi,\theta}, L_{\Psi}, L_{\Psi,\theta}\ge 0$ such that for all $t$, all $\mathbf{s},\mathbf{s}',\mathbf{u}$,
\begin{align}
\norm{\Phi_t(\mathbf{s},\mathbf{u};\boldsymbol{\theta})-\Phi_t(\mathbf{s}',\mathbf{u};\boldsymbol{\theta})}
&\le \rho\,\norm{\mathbf{s}-\mathbf{s}'},
\label{eq:contractive_assump}\\
\norm{\Phi_t(\mathbf{s},\mathbf{u};\boldsymbol{\theta})-\Phi_t(\mathbf{s},\mathbf{u};\hat{\boldsymbol{\theta}})}
&\le L_{\Phi,\theta}\,\norm{\Delta\boldsymbol{\theta}},
\label{eq:phi_theta_lip}\\
\norm{\Psi_t(\mathbf{s};\boldsymbol{\theta})-\Psi_t(\mathbf{s}';\boldsymbol{\theta})}
&\le L_{\Psi}\,\norm{\mathbf{s}-\mathbf{s}'},
\label{eq:psi_state_lip}\\
\norm{\Psi_t(\mathbf{s};\boldsymbol{\theta})-\Psi_t(\mathbf{s};\hat{\boldsymbol{\theta}})}
&\le L_{\Psi,\theta}\,\norm{\Delta\boldsymbol{\theta}}.
\label{eq:psi_theta_lip}
\end{align}
Then the output deviation is uniformly bounded for all $t$ as
\begin{equation}
\norm{\mathbf{y}_t-\hat{\mathbf{y}}_t}
\le
\left(\frac{L_{\Psi}L_{\Phi,\theta}}{1-\rho}+L_{\Psi,\theta}\right)\norm{\Delta\boldsymbol{\theta}}.
\label{eq:ssm_quant_bound}
\end{equation}
\end{proposition}

\begin{IEEEproof}
Define $e_t\triangleq \norm{\mathbf{s}_t-\hat{\mathbf{s}}_t}$. Using \eqref{eq:ssm_block_general} and triangle inequality,
\begin{align}
e_{t+1}
&=
\norm{\Phi_t(\mathbf{s}_t,\mathbf{u}_t;\boldsymbol{\theta})-\Phi_t(\hat{\mathbf{s}}_t,\mathbf{u}_t;\hat{\boldsymbol{\theta}})} \nonumber\\
&\le
\norm{\Phi_t(\mathbf{s}_t,\mathbf{u}_t;\boldsymbol{\theta})-\Phi_t(\hat{\mathbf{s}}_t,\mathbf{u}_t;\boldsymbol{\theta})}
+
\norm{\Phi_t(\hat{\mathbf{s}}_t,\mathbf{u}_t;\boldsymbol{\theta})-\Phi_t(\hat{\mathbf{s}}_t,\mathbf{u}_t;\hat{\boldsymbol{\theta}})} \nonumber\\
&\le \rho e_t + L_{\Phi,\theta}\norm{\Delta\boldsymbol{\theta}}.
\end{align}
Since $e_0=0$ and $\rho<1$, unrolling yields $e_t \le \frac{L_{\Phi,\theta}}{1-\rho}\norm{\Delta\boldsymbol{\theta}}$ for all $t$. Finally,
\begin{align}
\norm{\mathbf{y}_t-\hat{\mathbf{y}}_t}
&\le
\norm{\Psi_t(\mathbf{s}_t;\boldsymbol{\theta})-\Psi_t(\hat{\mathbf{s}}_t;\boldsymbol{\theta})}
+
\norm{\Psi_t(\hat{\mathbf{s}}_t;\boldsymbol{\theta})-\Psi_t(\hat{\mathbf{s}}_t;\hat{\boldsymbol{\theta}})} \nonumber\\
&\le L_{\Psi}e_t + L_{\Psi,\theta}\norm{\Delta\boldsymbol{\theta}}
\le
\left(\frac{L_{\Psi}L_{\Phi,\theta}}{1-\rho}+L_{\Psi,\theta}\right)\norm{\Delta\boldsymbol{\theta}},
\end{align}
which proves \eqref{eq:ssm_quant_bound}.
\end{IEEEproof}

\noindent\textbf{Interpretation.} The bound \eqref{eq:ssm_quant_bound} does not grow with token length because $\rho<1$ prevents perturbations from being amplified through recursion. For selective SSMs such as Mamba, $\Phi_t$ is input-dependent; the same conclusion holds under uniform contractivity and the stated regularity assumptions over the operating range.

\subsection{Structural Implications and Asymmetric Architecture}
\label{subsec:asym_conclude}
For delay--angular CSI exhibiting long-tailed leakage \eqref{eq:longtail_bound}, hard-locality encoders may incur structural truncation loss unless receptive fields are substantially expanded, because the residual tail energy decays only polynomially with receptive-field size \eqref{eq:hard_tail}. In contrast, SSM-based encoders provide soft locality: long-range components are not abruptly removed but recursively aggregated with exponentially decaying weights \eqref{eq:soft_tail_bound}, yielding a rapidly diminishing incremental influence from very distant coefficients under the same UE complexity regime. Moreover, when the state recursion is contractive, quantization-induced perturbations do not amplify with token length (Proposition~\ref{prop:quant_robust}), supporting robust UE-side low-precision inference.

Motivated by these observations and the encoder-induced floor in \eqref{eq:mse_decomp}, we adopt an asymmetric CSI feedback architecture that deploys a lightweight Mamba-style SSM encoder at the UE \cite{Mamba} and a higher-capacity Transformer-based decoder at the BS (in the spirit of TransNet \cite{TransNet}). The BS decoder aims to approximate the conditional mean $\E[\mathbf{X}_a\given \mathbf{Z}]$, refining reconstruction without affecting the encoder-induced floor.
...
SNR (dB) & 10 / 20 / 30 \\
Dataset & COST~2100 (outdoor, $f_c=300$~MHz) \cite{COST2100}; FR1 TDL-A/B/C \cite{3GPP38901} \\
\midrule
Model & Mamba (UE encoder) \cite{Mamba} + Transformer (BS decoder) \\
Compression ratio & $\mathrm{CR}=1/4$, $1/16$ \\
...
Saving set $\mathcal{S}$ & 0.75--0.95 (75--95\%) encoder BOP saving vs.\ FP32 (step 0.005) \\
...
\begin{table}[t]
\centering
\caption{Structural Performance ($\mathrm{CR}=1/4$)}
...
\midrule
CsiNet   & -8.75  &  2.17 &  10.89 \\
CLNet    & -12.87 &  1.51 &   4.54 \\
TransNet & -14.86 & 17.86 & 35.72 \\
\textbf{Mamba-Transformer AE} & \textbf{-15.34} & \textbf{4.87} & 22.74 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Structural Performance ($\mathrm{CR}=1/16$)}
\label{tab:struct_cr18_final}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccc}
\toprule
Model & NMSE (dB) & Enc. FLOPs (M) & Total FLOPs (M) \\
\midrule
CsiNet   & -4.51 & 0.60 & 7.74 \\
CLNet    & -5.56 & 0.73 & 2.97 \\
TransNet & \textemdash & \textemdash & \textemdash \\
\textbf{Mamba-Transformer AE} & \textemdash & \textemdash & \textemdash \\
\bottomrule
\end{tabular}
\end{table}
...
We evaluate uniform weight quantization under INT16/INT8/INT4 at $\mathrm{CR}=1/4$ and $\mathrm{CR}=1/16$.
Table~\ref{tab:uniform_quant_cr4} and Table~\ref{tab:uniform_quant_cr16} report NMSE and encoder BOPs for each precision level.
Fig.~\ref{fig:uniform_quant} visualizes the accuracy--efficiency trade-off: at $\mathrm{CR}=1/4$ the Mamba-Transformer AE retains near-lossless NMSE down to INT8 ($87.5\%$ BOPs saving), whereas the CNN baselines (CsiNet, CLNet) degrade sharply beyond INT16. Similar trends hold at $\mathrm{CR}=1/16$.
This robustness gap motivates the mixed-precision strategy developed in the subsequent sections.
...
\begin{tabular}{llccc}
\toprule
Model & Prec. & NMSE (dB) & Enc.\ BOPs (M) & Saving (\%) \\
\midrule
...
\midrule
TransNet & INT16 & \textemdash & \textemdash & 75.00 \\
         & INT8  & \textemdash & \textemdash & 87.50 \\
         & INT4  & \textemdash & \textemdash & 93.75 \\
\midrule
Mamba-Transformer AE & INT16 & $-15.37$ & 1245.46 & 75.00 \\
                     & INT8  & $-15.19$ &  622.85 & 87.50 \\
                     & INT4  &   0.03   &  311.47 & 93.75 \\
                     & INT2  &   7.69   &  155.78 & 96.88 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/fig_uniform_quant.pdf}
\caption{NMSE vs.\ BOPs saving under uniform weight quantization (solid: $\mathrm{CR}=1/4$, dashed: $\mathrm{CR}=1/16$).
The Mamba-Transformer AE maintains near-lossless reconstruction up to $87.5\%$ BOPs saving (INT8), while the CNN baselines degrade sharply beyond INT16.}
\label{fig:uniform_quant}
\end{figure}

\begin{table}[t]
\centering
\caption{Uniform quantization performance ($\mathrm{CR}=1/16$); encoder activations are fixed to INT16 and the listed precision refers to weight bit-width.}
\label{tab:uniform_quant_cr16}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccc}
\toprule
Model & Prec. & NMSE (dB) & Enc.\ BOPs (M) & Saving (\%) \\
...
CsiNet & INT16 & $-4.51$ & 67.18 & 75.00 \\
       & INT8  & $-4.31$ & 33.59 & 87.50 \\
       & INT4  &  16.74  & 16.80 & 93.75 \\
       & INT2  &  22.54  &  8.40 & 96.88 \\
\midrule
CLNet  & INT16 & $-5.55$ & 67.26 & 75.00 \\
       & INT8  &  13.62  & 33.63 & 87.50 \\
       & INT4  &  25.66  & 16.82 & 93.75 \\
       & INT2  &  22.63  &  8.41 & 96.88 \\
\midrule
TransNet & INT16 & \textemdash & \textemdash & 75.00 \\
         & INT8  & \textemdash & \textemdash & 87.50 \\
         & INT4  & \textemdash & \textemdash & 93.75 \\
\midrule
Mamba-Transformer AE & INT16 & \textemdash & \textemdash & 75.00 \\
                     & INT8  & \textemdash & \textemdash & 87.50 \\
                     & INT4  & \textemdash & \textemdash & 93.75 \\
\bottomrule
\end{tabular}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{kl_vs_ilp.pdf}
\caption{ILP-based ranking versus KL-refined policy evaluation under identical computational budgets.}
\label{fig:kl_vs_ilp}
\end{figure}
...
\subsubsection{Outage Comparison}
Fig.~\ref{fig:outage_final} presents outage probabilities under identical average encoder BOP budgets. Online RP-MPQ suppresses outage events more effectively than static policies without increasing average UE-side encoder complexity.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{outage_curves.pdf}
\caption{Outage probability comparison under identical average UE-side computational budgets. Top: static policies. Bottom: online RP-MPQ.}
...
\begin{table}[t]
\centering
\caption{Budget Consistency under Online RP-MPQ}
...
To evaluate structural generalization, we train on a mixed set of FR1 TDL-A/B/C realizations and evaluate separately on each profile without profile-specific tuning. Table~\ref{tab:tdl_generalization} reports profile-wise NMSE and encoder complexity. We also report measured runtime on a UE platform in Table~\ref{tab:runtime_validation}.

\begin{table}[t]
\centering
\caption{Profile-wise Generalization under FR1 TDL (CR=1/4)}
...
\midrule
NMSE (dB) & \textemdash & \textemdash & \textemdash \\
Encoder FLOPs (M) & \textemdash & \textemdash & \textemdash \\
Model size (MB) & \textemdash & \textemdash & \textemdash \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Measured Runtime of Online RP-MPQ on UE}
...
\midrule
Base encoder execution & \textemdash \\
Online precision selection & \textemdash \\
Total encoder latency & \textemdash \\
\bottomrule
\end{tabular}
\end{table}
